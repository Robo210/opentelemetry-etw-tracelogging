...
use opentelemetry_api::trace::{Tracer, Span};
...
fn main() {
    ...
    let span_builder = tracer.span_builder("my_cool_span").with_kind(opentelemetry::trace::SpanKind::Client).with_status(opentelemetry::trace::Status::Error { description: "asdf".into() });
